...
$baseurl = new moodle_url('/mod/bycs_feedback/show_entries.php', array('id' => $cm->id));
...
$actionbar = new \mod_bycs_feedback\output\responses_action_bar($cm->id, $baseurl);

if ($deleteid) {
    // This is a request to delete a reponse.
    require_capability('mod/feedback:deletesubmissions', $context);
    require_sesskey();
    $feedbackstructure = new mod_bycs_feedback_completion($feedback, $cm, 0, true, $deleteid);
    ...
    $feedbackstructure = new mod_bycs_feedback_completion($feedback, $cm, 0, true, $showcompleted, $userid);
} else {
    // Viewing list of reponses.
    $feedbackstructure = new mod_bycs_feedback_structure($feedback, $cm, $courseid);
}

$responsestable = new mod_bycs_feedback_responses_table($feedbackstructure);
$anonresponsestable = new mod_bycs_feedback_responses_anon_table($feedbackstructure);
...
$courseselectform = new mod_bycs_feedback_course_select_form($baseurl, $feedbackstructure, $feedback->course == SITEID);
...
/** @var \mod_bycs_feedback\output\renderer $renderer */
$renderer = $PAGE->get_renderer('mod_bycs_feedback');
echo $renderer->main_action_bar($actionbar);
echo $OUTPUT->heading(get_string('show_entries', 'mod_bycs_feedback'), 3);
...
if ($userid || $showcompleted) {
    ...
    $form = new mod_bycs_feedback_complete_form(mod_bycs_feedback_complete_form::MODE_VIEW_RESPONSE,
