...
$url = new moodle_url('/mod/bycs_feedback/show_nonrespondents.php', array('id'=>$cm->id));
...
$actionbar = new \mod_bycs_feedback\output\responses_action_bar($cm->id, $url);
...
if ($action == 'sendmessage' AND $canbulkmessaging) {
    ...
    $link2 = $CFG->wwwroot.'/mod/bycs_feedback/index.php?id='.$course->id;
    $link3 = $CFG->wwwroot.'/mod/bycs_feedback/view.php?id='.$cm->id;
...
$baseurl = new moodle_url('/mod/bycs_feedback/show_nonrespondents.php');
    ...
    foreach ($students as $student) {
        ...
        if ($canbulkmessaging) {
            $checkbox = new \core\output\checkbox_toggleall('feedback-non-respondents', false, [
                ...
                'classes' => 'mr-1',
                'value' => $student->id,
                'label' => get_string('includeuserinrecipientslist', 'mod_bycs_feedback', fullname($student)),
