 ...
 *
 * @global object
 * @param object $feedback the object given by mod_bycs_feedback_mod_form
 * @return int
 */
function feedback_add_instance($feedback) {
    ...
    if ($draftitemid = $feedback->page_after_submit_editor['itemid']) {
        $feedback->page_after_submit = file_save_draft_area_files($draftitemid, $context->id,
                                                    'mod_bycs_feedback', 'page_after_submit',
                                                    0, $editoroptions,
...
function feedback_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options=array()) {
    ...
    if ($filearea === 'page_after_submit') {
        $fullpath = "/{$context->id}/mod_bycs_feedback/$filearea/$relativepath";
    } else {
        $fullpath = "/{$context->id}/mod_bycs_feedback/$filearea/{$item->id}/$relativepath";
    }
...
function feedback_print_recent_mod_activity($activity, $courseid, $detail, $modnames) {
    ...
    if ($detail) {
        ...
        echo "<a href=\"$CFG->wwwroot/mod/bycs_feedback/view.php?id={$activity->cmid}\">{$activity->name}</a>";
        echo '</div>';
    }
...
function feedback_user_complete($course, $user, $mod, $feedback) {
    ...
    if ($completed = $DB->get_record('feedback_completed', $params)) {
        ...
        if (has_capability('mod/feedback:viewreports', $context)) {
            $url = new moodle_url('/mod/bycs_feedback/show_entries.php',
    ...
    foreach ($feedbackitems as $item) {
        ...
        if ($itemfiles) {
            foreach ($itemfiles as $ifile) {
                ...
                $file_record->component = 'mod_bycs_feedback';
...
function feedback_get_item_class($typ) {
    ...
    require_once($CFG->dirroot.'/mod/bycs_feedback/item/feedback_item_class.php');

    //get the class of item-typ
    $typeclean = clean_param($typ, PARAM_ALPHA);

    $itemclass = "feedback_item_{$typeclean}";
    $itemclasspath = "{$CFG->dirroot}/mod/bycs_feedback/item/{$typeclean}/lib.php";
...
function feedback_delete_item($itemid, $renumber = true, $template = false) {
    ...
    if ($template) {
        ...
        if ($templatefiles) {
            $fs->delete_area_files($context->id, 'mod_bycs_feedback', 'template', $item->id);
        }
    } else {
        ...
        if ($itemfiles) {
            $fs->delete_area_files($context->id, 'mod_bycs_feedback', 'item', $item->id);
        }
    }
...
function feedback_save_tmp_values($feedbackcompletedtmp, ?stdClass $feedbackcompleted = null) {
    ...
    $event = \mod_bycs_feedback\event\response_submitted::create_from_record($feedbackcompleted, $cm);
    $event->trigger();
...
function feedback_delete_completed($completed, $feedback = null, $cm = null, $course = null) {
    ...
    // Trigger event for the delete action we performed.
    $event = \mod_bycs_feedback\event\response_deleted::create_from_record($completed, $cm, $feedback);
    $event->trigger();
...
function feedback_send_email($cm, $feedback, $course, $user, $completed = null) {
    ...
    if ($teachers) {
        ...
        foreach ($teachers as $teacher) {
            ...
            $info->url = $CFG->wwwroot.'/mod/bycs_feedback/show_entries.php?'.
                            'id='.$cm->id.'&'.
                            'userid=' . $user->id;
            if ($completed) {
            ...
            if ($feedback->anonymous == FEEDBACK_ANONYMOUS_NO) {
                ...
                $eventdata->component        = 'mod_bycs_feedback';
...
function feedback_send_email_anonym($cm, $feedback, $course) {
    ...
    if ($teachers) {
        ...
        foreach ($teachers as $teacher) {
            ...
            $info->username = $printusername;
            $info->feedback = format_string($feedback->name, true);
            $info->url = $CFG->wwwroot.'/mod/bycs_feedback/show_entries.php?id=' . $cm->id;
...
function feedback_send_email_html($info, $course, $cm) {
    ...
    $feedback_all_url = $CFG->wwwroot.'/mod/bycs_feedback/index.php?id='.$course->id;
    $feedback_url = $CFG->wwwroot.'/mod/bycs_feedback/view.php?id='.$cm->id;
...
function feedback_extend_settings_navigation(settings_navigation $settings, navigation_node $feedbacknode) {
    ...
    if (has_capability('mod/feedback:edititems', $context)) {
        ...
        $questionnode->add(get_string('edit_items', 'feedback'),
            new moodle_url('/mod/bycs_feedback/edit.php', ['id' => $settings->get_page()->cm->id]));

        $questionnode->add(get_string('export_questions', 'feedback'),
            new moodle_url('/mod/bycs_feedback/export.php', ['id' => $settings->get_page()->cm->id, 'action' => 'exportfile']));

        $questionnode->add(get_string('import_questions', 'feedback'),
            new moodle_url('/mod/bycs_feedback/import.php', ['id' => $settings->get_page()->cm->id]));

        $feedbacknode->add(get_string('templates', 'feedback'),
            new moodle_url('/mod/bycs_feedback/manage_templates.php', ['id' => $settings->get_page()->cm->id, 'mode' => 'manage']),
    ...
    if (has_capability('mod/feedback:mapcourse', $context) && $settings->get_page()->course->id == SITEID) {
        $feedbacknode->add(get_string('mappedcourses', 'feedback'),
            new moodle_url('/mod/bycs_feedback/mapcourse.php', ['id' => $settings->get_page()->cm->id]),
    ...
    if ($feedback->course == SITEID) {
        $analysisnode = navigation_node::create(get_string('analysis', 'feedback'),
            new moodle_url('/mod/bycs_feedback/analysis_course.php', ['id' => $settings->get_page()->cm->id]),
            navigation_node::TYPE_CUSTOM, null, 'feedbackanalysis');
    } else {
        $analysisnode = navigation_node::create(get_string('analysis', 'feedback'),
            new moodle_url('/mod/bycs_feedback/analysis.php', ['id' => $settings->get_page()->cm->id]),
            navigation_node::TYPE_CUSTOM, null, 'feedbackanalysis');
    }
    ...
    if (has_capability('mod/feedback:viewreports', $context)) {
        ...
        $feedbacknode->add(get_string(($hassecondary ? 'responses' : 'show_entries'), 'feedback'),
            new moodle_url('/mod/bycs_feedback/show_entries.php', ['id' => $settings->get_page()->cm->id]),
            navigation_node::TYPE_CUSTOM, null, 'responses');
    } else {
        $feedbackcompletion = new mod_bycs_feedback_completion($feedback, $context, $settings->get_page()->course->id);
 ...
 */
function mod_bycs_feedback_get_fontawesome_icon_map() {
    return [
        'mod_bycs_feedback:required' => 'fa-exclamation-circle',
        'mod_bycs_feedback:notrequired' => 'fa-question-circle-o',
    ];
}
 ...
 */
function mod_bycs_feedback_core_calendar_provide_event_action(calendar_event $event,
    ...
    $feedbackcompletion = new mod_bycs_feedback_completion(null, $cm, 0, false, null, null, $userid);
    ...
    return $factory->create_instance(
        ...
        new \moodle_url('/mod/bycs_feedback/view.php', ['id' => $cm->id]),
        1,
        $actionable
    );
}
 ...
 */
function mod_bycs_feedback_get_completion_active_rule_descriptions($cm) {
 ...
 * @return array
 */
function mod_bycs_feedback_core_calendar_get_valid_event_timestart_range(\calendar_event $event, \stdClass $instance) {
    $mindate = null;
    $maxdate = null;
 ...
 */
function mod_bycs_feedback_core_calendar_event_timestart_updated(\calendar_event $event, \stdClass $feedback) {
    global $CFG, $DB;
 ...
 */
function mod_bycs_feedback_core_calendar_get_event_action_string(string $eventtype): string {
