...
/**
 * Contains class mod_bycs_feedback_complete_form
 *
...
/**
 * Class mod_bycs_feedback_complete_form
 *
 * @package   mod_bycs_feedback
 * @copyright 2016 Marina Glancy
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_bycs_feedback_complete_form extends moodleform {
    ...
    /** @var int */
    protected $mode;
    /** @var mod_bycs_feedback_structure|mod_bycs_feedback_completion */
    protected $structure;
    /** @var mod_bycs_feedback_completion */
     ...
     *
     * @param int $mode
     * @param mod_bycs_feedback_structure $structure
     * @param string $formid CSS id attribute of the form
     * @param array $customdata
     */
    public function __construct($mode, mod_bycs_feedback_structure $structure, $formid, $customdata = null) {
        $this->mode = $mode;
    ...
    protected function definition_complete() {
        if (!$this->structure instanceof mod_bycs_feedback_completion) {
    ...
    protected function add_item_label($item, $element) {
        if (strlen($item->label) && ($this->mode == self::MODE_EDIT || $this->mode == self::MODE_VIEW_TEMPLATE)) {
            $name = get_string('nameandlabelformat', 'mod_bycs_feedback',
    ...
    public function display() {
        ...
        if ($this->mode == self::MODE_EDIT) {
            $PAGE->requires->js_call_amd('mod_bycs_feedback/edit', 'setup');
        }
    }
}
