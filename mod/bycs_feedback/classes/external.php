...
use mod_bycs_feedback\external\feedback_summary_exporter;
use mod_bycs_feedback\external\feedback_completedtmp_exporter;
use mod_bycs_feedback\external\feedback_item_exporter;
use mod_bycs_feedback\external\feedback_valuetmp_exporter;
use mod_bycs_feedback\external\feedback_value_exporter;
use mod_bycs_feedback\external\feedback_completed_exporter;
 ...
 * @since      Moodle 3.3
 */
class mod_bycs_feedback_external extends external_api {
    ...
    protected static function validate_feedback($feedbackid, $courseid = 0) {
        ...
        if ($feedbackcourse->id == SITEID && $courseid) {
            ...
            $feedbackcompletion = new mod_bycs_feedback_completion($feedback, $cm, $courseid);
            if (!$feedbackcompletion->check_course_is_mapped()) {
                throw new moodle_exception('cannotaccess', 'mod_bycs_feedback');
            }
        }
     ...
     * @throws moodle_exception
     * @return mod_bycs_feedback_completion feedback completion instance
     * @since  Moodle 3.3
     */
    protected static function validate_feedback_access($feedback, $course, $cm, $context, $checksubmit = false) {
        $feedbackcompletion = new mod_bycs_feedback_completion($feedback, $cm, $course->id);
    ...
    public static function get_feedback_access_information($feedbackid, $courseid = 0) {
        ...
        list($feedback, $course, $cm, $context, $completioncourse) = self::validate_feedback($params['feedbackid'],
            $params['courseid']);
        $feedbackcompletion = new mod_bycs_feedback_completion($feedback, $cm, $completioncourse->id);
    ...
    public static function get_items($feedbackid, $courseid = 0) {
        ...
        if ($userhasaccess || has_any_capability($capabilities, $context)) {
            ...
            $warnings = [];
            $feedbackstructure = new mod_bycs_feedback_structure($feedback, $cm, $completioncourse->id);
    ...
    public static function get_page_items($feedbackid, $page, $courseid = 0) {
        ...
        if ($userhasaccess || has_any_capability($capabilities, $context)) {
            ...
            $warnings[] = [
                'item' => $feedback->id,
                'warningcode' => 'nopermission',
                'message' => get_string('nopermission', 'mod_bycs_feedback'),
            ];
        }
    ...
    public static function process_page($feedbackid, $page, $responses = [], $goprevious = false, $courseid = 0) {
        ...
        $_POST['_qf__mod_bycs_feedback_complete_form'] = 1;
    ...
    public static function get_analysis($feedbackid, $groupid = 0, $courseid = 0) {
        ...
        // Summary data.
        $summary = new mod_bycs_feedback\output\summary($feedbackstructure, $groupid);
    ...
    public static function get_responses_analysis($feedbackid, $groupid = 0, $page = 0, $perpage = 0, $courseid = 0) {
        ...
        $feedbackstructure = new mod_bycs_feedback_structure($feedback, $cm, $completioncourse->id);
        $responsestable = new mod_bycs_feedback_responses_table($feedbackstructure, $groupid);
        // Ensure responses number is correct prior returning them.
        $feedbackstructure->shuffle_anonym_responses();
        $anonresponsestable = new mod_bycs_feedback_responses_anon_table($feedbackstructure, $groupid);
