 ...
 */
namespace mod_bycs_feedback;

use mod_bycs_feedback_completion;
...
class lib_test extends \advanced_testcase {
    ...
    public function test_feedback_initialise() {
        ...
        foreach ($constructorparams as $params) {
            $structure = new mod_bycs_feedback_completion($params[0], $params[1], 0);
    ...
    /**
     * Tests for mod_bycs_feedback_refresh_events.
     */
    public function test_feedback_refresh_events() {
        ...
        $course = $this->getDataGenerator()->create_course();
        $generator = $this->getDataGenerator()->get_plugin_generator('mod_bycs_feedback');
        $params['course'] = $course->id;
    ...
    public function test_feedback_core_calendar_provide_event_action_open() {
        ...
        $actionevent = mod_bycs_feedback_core_calendar_provide_event_action($event, $factory);
    ...
    public function test_feedback_core_calendar_provide_event_action_open_for_user() {
        ...
        $actionevent = mod_bycs_feedback_core_calendar_provide_event_action($event, $factory, $user2->id);
        $this->assertNull($actionevent);

        // User2 checking $user's events.
        $actionevent = mod_bycs_feedback_core_calendar_provide_event_action($event, $factory, $user->id);
        $this->assertInstanceOf('\core_calendar\local\event\value_objects\action', $actionevent);
        $this->assertEquals(get_string('answerquestions', 'feedback'), $actionevent->get_name());
        $this->assertInstanceOf('moodle_url', $actionevent->get_url());
        $this->assertEquals(1, $actionevent->get_item_count());
    ...
    public function test_feedback_core_calendar_provide_event_action_can_not_submit() {
        ...
        $factory = new \core_calendar\action_factory();
        $action = mod_bycs_feedback_core_calendar_provide_event_action($event, $factory);
    ...
    public function test_feedback_core_calendar_provide_event_action_already_completed_for_user() {
        ...
        $actionevent = mod_bycs_feedback_core_calendar_provide_event_action($event, $factory, $student->id);
     ...
     */
    public function test_mod_bycs_feedback_completion_get_active_rule_descriptions() {
        $this->resetAfterTest();
        $this->setAdminUser();
        ...
        $this->assertEquals(mod_bycs_feedback_get_completion_active_rule_descriptions($cm1), $activeruledescriptions);
        $this->assertEquals(mod_bycs_feedback_get_completion_active_rule_descriptions($cm2), []);
        $this->assertEquals(mod_bycs_feedback_get_completion_active_rule_descriptions($moddefaults), $activeruledescriptions);
        $this->assertEquals(mod_bycs_feedback_get_completion_active_rule_descriptions(new \stdClass()), []);
    }
    ...
    public function test_get_valid_event_timestart_range_unknown_event() {
        ...
        list($min, $max) = mod_bycs_feedback_core_calendar_get_valid_event_timestart_range($event, $feedback);
        $this->assertNull($min);
        $this->assertNull($max);
    }
     ...
     */
    public function test_mod_bycs_feedback_core_calendar_event_timestart_updated_unknown_event() {
        global $CFG, $DB;
        require_once($CFG->dirroot . "/calendar/lib.php");
        ...
        mod_bycs_feedback_core_calendar_event_timestart_updated($event, $feedback);
     ...
     */
    public function test_mod_bycs_feedback_core_calendar_event_timestart_updated_open_event() {
        global $CFG, $DB;
        require_once($CFG->dirroot . "/calendar/lib.php");
     ...
     * the feedback activity.
     */
    public function test_mod_bycs_feedback_core_calendar_event_timestart_updated_close_event() {
        global $CFG, $DB;
        require_once($CFG->dirroot . "/calendar/lib.php");
    ...
    public function test_creation_with_no_calendar_capabilities() {
        ...
        $generator = self::getDataGenerator()->get_plugin_generator('mod_bycs_feedback');
