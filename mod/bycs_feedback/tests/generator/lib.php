...
/**
 * mod_bycs_feedback data generator.
 *
...
/**
 * mod_bycs_feedback data generator class.
 *
 * @package    mod_bycs_feedback
 * @category   test
 * @copyright  2013 Ankit Agarwal
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_bycs_feedback_generator extends testing_module_generator {
    ...
    public function create_response(array $data): stdClass {
        ...
        $feedbackcompletion = new mod_bycs_feedback_completion(
            $feedback,
            $cm,
            $cm->course,
            false,
    ...
    protected function format_item_values(string $questiontype, string $values): string {
        ...
        if (!file_exists($CFG->dirroot.'/mod/bycs_feedback/item/'.$questiontype.'/lib.php')) {
            throw new coding_exception("Question type '$questiontype' not found");
        }

        require_once($CFG->dirroot.'/mod/bycs_feedback/item/'.$questiontype.'/lib.php');
