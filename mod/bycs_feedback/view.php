...
$PAGE->set_url('/mod/bycs_feedback/view.php', array('id' => $cm->id));
...
if (!has_capability('mod/feedback:edititems', $context) &&
    ...
    echo $OUTPUT->header();
    echo $OUTPUT->notification(get_string('cannotaccess', 'mod_bycs_feedback'));
    echo $OUTPUT->footer();
    exit;
}

$viewcompletion = $feedbackcompletion->is_open() && $feedbackcompletion->can_complete() && $feedbackcompletion->can_submit();
$actionbar = new \mod_bycs_feedback\output\standard_action_bar(
    $cm->id,
    $viewcompletion,
...
$previewlnk = new moodle_url('/mod/bycs_feedback/print.php', array('id' => $id));
if ($courseid) {
...
if (has_capability('mod/feedback:edititems', $context)) {
    ...
    //get the groupid
    $groupselect = groups_print_activity_menu($cm, $CFG->wwwroot.'/mod/bycs_feedback/view.php?id='.$cm->id, true);
    $mygroupid = groups_get_activity_group($cm);

    echo $groupselect.'<div class="clearer">&nbsp;</div>';
    $summary = new mod_bycs_feedback\output\summary($feedbackcompletion, $mygroupid);
    echo $OUTPUT->render_from_template('mod_bycs_feedback/summary', $summary->export_for_template($OUTPUT));
...
if (!$PAGE->has_secondary_navigation()) {
    if (!has_capability('mod/feedback:viewreports', $context) &&
        ...
        $analysisurl = new moodle_url('/mod/bycs_feedback/analysis.php', array('id' => $id));
    ...
    if (has_capability('mod/feedback:mapcourse', $context) && $feedback->course == SITEID) {
        ...
        $mapurl = new moodle_url('/mod/bycs_feedback/mapcourse.php', array('id' => $id));
