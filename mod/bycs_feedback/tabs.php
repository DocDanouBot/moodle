...
$viewurl = new moodle_url('/mod/bycs_feedback/view.php', array('id' => $usedid));
...
if (has_capability('mod/feedback:edititems', $context)) {
    $editurl = new moodle_url('/mod/bycs_feedback/edit.php', $urlparams + ['do_show' => 'edit']);
    $row[] = new tabobject('edit', $editurl->out(), get_string('edit_items', 'feedback'));

    $templateurl = new moodle_url('/mod/bycs_feedback/edit.php', $urlparams + ['do_show' => 'templates']);
...
if ($feedback->course == SITEID && has_capability('mod/feedback:mapcourse', $context)) {
    $mapurl = new moodle_url('/mod/bycs_feedback/mapcourse.php', $urlparams);
    $row[] = new tabobject('mapcourse', $mapurl->out(), get_string('mappedcourses', 'feedback'));
}

if (has_capability('mod/feedback:viewreports', $context)) {
    if ($feedback->course == SITEID) {
        $analysisurl = new moodle_url('/mod/bycs_feedback/analysis_course.php', $urlparams);
    } else {
        $analysisurl = new moodle_url('/mod/bycs_feedback/analysis.php', $urlparams);
    }
    $row[] = new tabobject('analysis', $analysisurl->out(), get_string('analysis', 'feedback'));

    $reporturl = new moodle_url('/mod/bycs_feedback/show_entries.php', $urlparams);
    ...
    if ($feedback->anonymous == FEEDBACK_ANONYMOUS_NO AND $feedback->course != SITEID) {
        $nonrespondenturl = new moodle_url('/mod/bycs_feedback/show_nonrespondents.php', $urlparams);
